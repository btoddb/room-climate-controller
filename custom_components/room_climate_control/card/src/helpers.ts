...
export function callScript(
  ...
  return hass.callService("script", script, {
...
export function isToggleOn(hass: HomeAssistant, entityId: string): boolean {
...
function findCardContent(root: ParentNode): HTMLElement | null {
  const haCard = root.querySelector("ha-card");
  if (haCard?.shadowRoot) {
    const inHaCard = haCard.shadowRoot.querySelector(".card-content");
    if (inHaCard instanceof HTMLElement) return inHaCard;
  }

  const direct = root.querySelector(".card-content");
  ...
  for (const el of elements) {
    ...
    const nested = findCardContent(el.shadowRoot);
...
function tryAppendToCardContent(
  card: HTMLElement,
  child: HTMLElement
): boolean {
  ...
  if (card.shadowRoot) {
    const content = findCardContent(card.shadowRoot);
    if (content) {
  ...
  if (card.tagName === "HA-CARD" && card.shadowRoot) {
...
/** Append into ha-card .card-content after the card is connected and rendered. */
export async function appendInsideCardContent(
  card: HTMLElement,
  child: HTMLElement,
  timeoutMs = 2500
): Promise<boolean> {
  ...
  if (tryAppendToCardContent(card, child)) return true;
  ...
  return new Promise((resolve) => {
    ...
    const attempt = () => {
      if (tryAppendToCardContent(card, child)) {
        finish(true);
        return true;
      }
      return false;
    ...
    const timer = window.setTimeout(() => finish(attempt()), timeoutMs);
  });
}

/** Inline styles — required when the button lives inside ha-card shadow DOM. */
export function applyInsetDeviceButtonStyles(
  wrap: HTMLElement,
