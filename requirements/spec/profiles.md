...
- **PR-1** `id` (canonical 2-digit, e.g. `08`), `name` (display), `room` (room key), `enabled`, `time` (`HH:MM`, 24h), `fan_override` (bool), `fan_reverse` (bool), `fan_preset` (`str | None`), and a per-device `presets` map.
...
- **PR-13** A profile's `fan_preset` is applied the same way: the apply writes the room's **Fan preset** select (when the room has a standalone fan), and the controller reacts per CC-26. The per-profile selector is exposed as a `select.*` entity; `None` is stored as `Auto` on apply.
...
- **PR-8** **Copy** puts a profile's settings (presets + fan override + fan reverse + fan preset) on the clipboard. **Name and time are never copied.**
